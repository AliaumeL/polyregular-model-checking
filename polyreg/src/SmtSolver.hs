...
toMona (FTag x tag) = x ++ " in T" ++ show tag
toMona (FLetter x letter) = x ++ " in L" ++ show letter

encodeMona :: [String] -> [String] -> Formula String Char -> String
encodeMona alphabet tags formula = unlines $ [preamble, alphabetVarsDef, tagsVarsDef, wordVars, labelsAt, labelsUnion, wordUnion, disjoint, covering, formula']
    where
        preamble = "m2l-str;"
        ...
        labelsAt :: String
        labelsAt = concat [labelAtI i name | (i, name) <- zip [0..] tagsVars ]
...
parseMonaOutput output = if "Formula is valid" `elem` lines output then Sat else if "Formula is unsatisfiable" `elem` lines output then Unsat else Unknown

-- use TmpDir to store the file
runMona :: String -> IO SmtResult
runMona input = do
    writeFile "tmp.mona" input
    output <- readProcess "mona" ["tmp.mona"] ""
