...
\AP For instance, the function \texttt{getIndex(l, i)} that returns the $i$-th
element of the list $l$ has type $(\TOut[n+1],1) \to \TOut[n]$, where $n$ is
the nesting depth of the list $l$.

\begin{verbatim}
def getBetweenIndicesBeforeStop(l, i, j): 
    seen_stop = False             # initialize a variable to false
    for (k, x) in enumerate(l):   # enumerate over a list 
        if i <= k and k <= j:     # compare indices 
            if x == "stop":       # compare lists to constants
                seen_stop = True  # set booleans to true
            if not seen_stop:     # boolean expressions
                yield x           # yielding elements
\end{verbatim}
