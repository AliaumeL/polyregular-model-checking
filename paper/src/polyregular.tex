...
In this section we provide two equivalent definitions of the \kl{first-order polyregular functions}: 
one using \kl{first-order simple for-programs} \cite[p. 19]{bojanczyk2018polyregular} and
one using the logical model of \kl{first-order string-to-string interpretations} \cite[Definition 4]{bojanczyk2019string}.
...
\kl{first-order simple for-programs} into \kl{first-order string-to-string interpretations}
...
\texttt{a}, \texttt{b} and the rest. In fact, all \kl{simple for programs} will have a similar property. 
We will use this observation to simplify the definition of
\kl{first-order string-to-string interpretations}, whose size would normally depend on the size of the alphabet.

Formally, we model the Unicode alphabet as an infinite set $\mathcal{D}$, and we define
a transduction $T : \mathcal{D}^* \to \mathcal{D}^*$ to be \intro{supported by} a set $A \subseteq \mathcal{D}$, if
...
It is not hard to see that the transductions defined by \kl{high-level programs} (of type $\TOut[1] \to \TOut[1]$) describe functions
that are supported by the finite set $A$ of letter constants that they use. This is also going to be the case for the \kl{simple for programs}.
In \cref{subsec:fo-string-to-string}, we will define a version of the \kl{first-order string-to-string interpretations}
