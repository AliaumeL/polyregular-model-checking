...
\begin{proof}
    ...
    that does not \kl{touch} elements of $A$, the truth value of $\varphi$ is the same for $w$ and $f^*(w)$. (Remember that 
...
\item An output function $\mathsf{out} : T \to A + \{1, \ldots, \mathsf{ar}(t)\}$. 
\item A domain formula $\varphi_{\mathsf{dom}}^t(x_1,\ldots,x_{\mathsf{ar}(t)})$ 
    for every tag $t \in T$.
\item An order formula $\varphi_{\leq}^{t,t'}(x_1,\ldots,x_{\mathsf{ar}(t)},y_1,\ldots,y_{\mathsf{ar}(t')})$ for every $t,t' \in T$. 
\end{enumerate}
...
An important property of the \kl{first-order string-to-string interpretations} is that they allow a direct 
reduction of the following \kl{Hoare triple verification} problem to the \kl{emptiness problem} for the \kl{first-order logic on words}. 
\begin{definition}
    The \intro{Hoare triple verification problem} for a \kl{first-order string-to-string interpretation} is the following decision problem: 
    \[
        \begin{tabular}{ll}
        ...
        \end{tabular}
    \]
    The instance of this problem is called \intro{Hoare triple} and it is denoted as $\{ \varphi \}\; F\; \{ \psi \}$. 
    We say that a \kl{Hoare triple} is \intro{valid} to the decision problem is positive.
\end{definition}
...
is already known \cite[Theorem~1.7]{bojanczyk2018polyregular}. However, to the best of our knowledge, 
no effort has been made so far to make this result practical. In the next section, we show how to construct a formula 
$\chi(\phi, F, \psi)$ that is \kl{empty} if and only if the triple $\{ \phi \}\; F\; \{ \psi \}$ is valid.
Moreover, the size and the quantifier depth of $\chi$ can be bound by the following low-degree polynomials:
\[
\begin{tabular}{cc}
    $\qrank(\chi) \leq \max\left(\qrank(\phi), \text{arity}(F) + 1) \cdot \qrank(\psi) + \qrank(F)\right)$ &
    $|\chi| \leq \max\left(|\phi|, (|F| + \qrank(F)) \cdot |\psi|\right)$
\end{tabular}
\]