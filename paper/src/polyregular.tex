...
extended version \kl{first-order simple for-programs}.} version of the \kl{for
programs}. The main difference is that the \kl{simple
...
that defines functions $\mathcal{D}^* \to \mathcal{D}^*$. It is based on the \intro{first-order logic on words},
...
\AP
An important property of this logic, it that it has decidable \intro{emptiness}, i.e. given a formula
...
is that they admit a direct reduction of the first-order Hoare triple
...
the construction resulting from this proof is not practical to implement.
